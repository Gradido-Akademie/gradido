import {
  ...
  MatchingEntry as DbMatchingEntry,
  User as DbUser,
} from 'database'
...
export async function sendUsersToGms(
  ...
  try {
    ...
    let entriesByUser: Map<number, DbMatchingEntry[]> | undefined
...
async function findLiveEntriesByUser(userIds: number[]): Promise<Map<number, DbMatchingEntry[]>> {
  ...
  const byUser = new Map<number, DbMatchingEntry[]>()
