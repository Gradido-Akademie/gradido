...
// Regression guard for the GMS/HumHub "connection dropped" bug: the token-handoff
// re-auth in routes/guards.js feeds the verifyLogin result into the shared `login`
// store action. If any of these fields is missing from the query, the action
// overwrites the value with undefined, so GMS/HumHub disconnect on every
// wallet <-> admin round-trip. verifyLogin must stay in sync with the login
// mutation for the fields the login action consumes.
